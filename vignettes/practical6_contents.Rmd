...
Then, when we land on a square we simply increase the associated
...
since ‘Old Kent Road’ is square 2 (see table \ref{T_1}).
...
simply going round the board. The code in the `SimulateMonopoly`
...
This function takes in the `current` position, with probability 1/16
...
`SimulateMonopoly()` function to incorporate the `CommunityChest` function:
...
What is the rationale for lines 14-20?
...
into the `SimulateMonopoly` code.
...
3. Square 31 is ‘Go To Jail.’ Implement this in your main `SimulateMonopoly()` function.
